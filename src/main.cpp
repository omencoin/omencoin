...
const string strMessageMagic = "Omencoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(int nHeight, int64_t nCoinAge, int64_t nFees)
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
           ...
           if(nBestHeight <= 100)
            {
            nSubsidy >>= nSubsidy /100000;  //no substantial pos reward until block 100
            }

            else if(nBestHeight <= 10000)
            {
            nSubsidy = nSubsidy * 0.5 ;  //500% until block 10000
            }
			
			else if(nBestHeight <= 20000)
            {
            nSubsidy = nSubsidy * 1 ;  //1000% until block 20000
            }
			
			else if(nBestHeight <= 25000)
            {
            nSubsidy = nSubsidy * 2;  //2000% until block 25000
            }
			
			else if(nBestHeight <= 30000)
            {
            nSubsidy = nSubsidy * 3 ;  //3000% until block 30000
            }
			
			else if(nBestHeight <= 31000)
            {
            nSubsidy = nSubsidy * 6;  //6000% until block 31000
            }
			
			else if(nBestHeight <= 32000)
            {
            nSubsidy = nSubsidy * 12 ;  //12000% until block 32000
            }
			
			else if(nBestHeight <= 50000)
            {
            nSubsidy = nSubsidy * 1 ;  //1000% until block 50000
            }
			
			else if(nBestHeight <= 51000)
            {
            nSubsidy = nSubsidy * 6 ;  //6000% until block 51000
            }
			
			else if(nBestHeight <= 52000)
            {
            nSubsidy = nSubsidy * 12 ;  //12000% until block 52000
            }
			
			else if(nBestHeight <= 80000)
            {
            nSubsidy = nSubsidy * 0.5 ;  //500% until block 80000
            }
			
			else if(nBestHeight <= 85000)
            {
            nSubsidy = nSubsidy * 1 ;  //1000% until block 85000
            }
			
			else if(nBestHeight <= 90000)
            {
            nSubsidy = nSubsidy * 2 ;  //2000% until block 90000
            }
			
			else if(nBestHeight <= 98000)
            {
            nSubsidy = nSubsidy * 3 ;  //3000% until block 98000
            }
			
			else if(nBestHeight <= 99000)
            {
            nSubsidy = nSubsidy * 6 ;  //6000% until block 99000
            }
			
			else if(nBestHeight <= 100000)
            {
            nSubsidy = nSubsidy * 12 ;  //12000% until block 100000
            }
			
			
			else if(nBestHeight <= 8000000)
            {
            nSubsidy = nSubsidy * 0.01 ;  //10% until block 8000000
            }

            else
			{
			 nSubsidy = nSubsidy * 0.01 ;  //10% until forever
			}
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Omencoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Typhoon slams Philippines as Mangkhut claims first victims";
        CTransaction txNew;
        txNew.nTime = 1537007465;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1537007465;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 4156982 : 4156982;
        ...
        assert(block.hashMerkleRoot == uint256("0x994fbc7657e350d615765b3df5e0ce0f30c41476b90819148c8d77a6722aa6ba"));
...
unsigned char pchMessageStart[4] = { 0x72, 0xa3, 0x72, 0x64 };
